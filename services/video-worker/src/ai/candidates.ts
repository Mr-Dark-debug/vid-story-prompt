...
import { aiClipCandidateSchema } from "./schema.js";

export type Candidate = z.infer<typeof aiClipCandidateSchema>;
...
const sentenceEnd = /[.!?]["')\]]?$/;
const hookPattern =
  /\b(why|how|what|never|stop|start|secret|mistake|imagine|here'?s|first|best|worst|\d+)\b/i;
...
function scoreWindow(words: TranscriptWord[], instruction: string) {
  ...
  const hookScore = clampScore(
    48 + (hookPattern.test(opening) ? 34 : 0) + (/[?!]/.test(opening) ? 12 : 0),
  );
  const clarityScore = clampScore(
    55 + (sentenceEnd.test(excerpt) ? 25 : 0) + (density >= 1.2 ? 12 : 0),
  );
  const standaloneScore = clampScore(
    50 + (sentenceEnd.test(excerpt) ? 20 : 0) + (words.length >= 35 ? 15 : 0),
  );
  const storyScore = clampScore(
    48 + (storyPattern.test(excerpt) ? 32 : 0) + (words.length >= 50 ? 10 : 0),
  );
...
export function buildCandidateWindows(input: {
  ...
  for (let index = 1; index < words.length; index++) {
    ...
    if (words[index].start - previous.end >= 1.1 || sentenceEnd.test(previous.text)) {
...
export function fallbackCandidate(window: CandidateWindow): Candidate {
  ...
  return aiClipCandidateSchema.parse({
