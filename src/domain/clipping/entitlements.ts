...
export type PlanEntitlement = {
  ...
  maxConnectorConnections: number;
  maxAutomationRules: number;
  s3ConnectorEnabled: boolean;
...
export const PLAN_ENTITLEMENTS: Readonly<Record<PlanKey, PlanEntitlement>> = {
  free: {
    key: "free",
    monthlySourceSeconds: 3600,
    maxSourceSecondsPerJob: 1800,
    maxClipsPerJob: 5,
    maxConcurrentJobs: 1,
    maxExport: { width: 1280, height: 720, fps: 30 },
    watermarkRequired: true,
    trialUnwatermarkedExports: 1,
    retentionDays: 7,
    maxRepromptsPerJob: 1,
    brandPresetLimit: 0,
    maxConnectorConnections: 1,
    maxAutomationRules: 0,
    s3ConnectorEnabled: false,
  ...
  creator: {
    key: "creator",
    monthlySourceSeconds: 36000,
    maxSourceSecondsPerJob: 7200,
    maxClipsPerJob: 20,
    maxConcurrentJobs: 2,
    maxExport: { width: 1920, height: 1080, fps: 30 },
    watermarkRequired: false,
    trialUnwatermarkedExports: 0,
    retentionDays: 30,
    maxRepromptsPerJob: 5,
    brandPresetLimit: 1,
    maxConnectorConnections: 3,
  ...
  pro: {
    key: "pro",
    monthlySourceSeconds: 108000,
    maxSourceSecondsPerJob: 21600,
    maxClipsPerJob: 50,
    maxConcurrentJobs: 4,
    maxExport: { width: 3840, height: 2160, fps: 60 },
    watermarkRequired: false,
    trialUnwatermarkedExports: 0,
    retentionDays: 90,
    maxRepromptsPerJob: 20,
    brandPresetLimit: 5,
    maxConnectorConnections: 10,
    maxAutomationRules: 10,
    s3ConnectorEnabled: true,
...
export function evaluateConnectorEntitlement(input: {
  ...
  if (input.connectorId === "s3" && !plan.s3ConnectorEnabled)
    return { allowed: false as const, reason: "s3_requires_pro" as const };
  if (input.connectedAccounts >= plan.maxConnectorConnections)
    return { allowed: false as const, reason: "connection_limit" as const };
  if (input.automationRequested && plan.maxAutomationRules === 0)
    return { allowed: false as const, reason: "automation_unavailable" as const };
  if (input.automationRequested && input.automationRules >= plan.maxAutomationRules)
    return { allowed: false as const, reason: "automation_limit" as const };
...
export function evaluateJobEntitlement(input: {
  plan: PlanKey;
