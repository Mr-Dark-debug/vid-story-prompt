...
export class YouTubeUrlError extends Error {
  constructor(public readonly code: "invalid_url" | "unsupported_url" | "invalid_video_id") { super(code); }
}

export function parseYouTubeVideoId(input: string) {
  let url: URL;
  try { url = new URL(input.trim()); } catch { throw new YouTubeUrlError("invalid_url"); }
  if (url.protocol !== "https:" && url.protocol !== "http:") throw new YouTubeUrlError("invalid_url");
